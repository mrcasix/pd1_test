<?php
	...
	if(isset($_REQUEST['idval']))
		$id_valutazione = base64_decode($_REQUEST['idval']);
	...
	$tab_esplicativa =get_table('dom_profili_ruolo dp,dom_descrizione_livelli dl', 'dl.*', 'dp.fk_id_dom_criteri_valutazione=dl.fk_id_dom_criteri_valutazione and dp.id_dom_profili_ruolo='.$id_profilo);
...
<div class="col-xs-12">
	<div class="row">
		<div class="col-sm-12 area_stampa">
			...
			$vertici_radar_risultato = array();
			$vertici_radar_due_risultato = array();
			$vertici_radar_competenze_tecnico_specialistiche = array();
			$somme_array = array();
			$somme_array_risultato = array();
			$index=0;
			...
			if(isset($elenco_competenze[$id_profilo])):
				foreach($elenco_competenze[$id_profilo] as $id_tipo=>$competenze):
					...
					if(isset($_REQUEST['idval']))
						$info_tipo_valutazione = get_table('dom_profili_utenti_valutazioni dpuv,dom_profili_utenti_valutazioni_valori dpuvv,dom_criteri_valutazione', '*', 'dpuvv.fk_id_dom_profili_utenti_valutazioni=dpuv.id_dom_profili_utenti_valutazioni and dpuv.id_dom_profili_utenti_valutazioni='.$id_valutazione.' and dpuvv.fk_id_dom_competenze='.$competenze[0]['id_comp']);
					
					
			
					echo '<div class="well well-sm green center"><b>'.strtoupper($info_tipo[0]['desc_tipo']).'</b></div>';
					$tot_atteso = 0;
					$tot_atteso_valutazione = 0;
					$n_comp = 0;
					$index=0;
					foreach($competenze as $info_competenza):
						...
						if($id_tipo==3){
						   ...
						   if(!isset($somme_array_risultato[$info_competenza["id_area"]])) {
							$somme_array_risultato[$info_competenza["id_area"]]["somma"]=0;
							$somme_array_risultato[$info_competenza["id_area"]]["conteggio"]=0;
							$somme_array_risultato[$info_competenza["id_area"]]["titolo"]=$info_competenza["id_area"];
						   }
						   if(isset($info_tipo_valutazione[$index]['valore'])){
								$somme_array_risultato[$info_competenza["id_area"]]["somma"]+=$info_tipo_valutazione[$index]['valore'];
								$somme_array_risultato[$info_competenza["id_area"]]["conteggio"]++;
						   }
						}
						...
						if(isset($info_tipo_valutazione[$index]['valore']))
							$tot_atteso_valutazione += $info_tipo_valutazione[$index]['valore'];
					...
					$media_atteso_valutazione = round($tot_atteso_valutazione/$n_comp, 1);
					$vertici_radar[] = array(strtoupper($info_tipo[0]['desc_tipo']), $media_atteso);
					$vertici_radar_risultato[] = array(strtoupper($info_tipo[0]['desc_tipo']), $media_atteso_valutazione);
				endforeach;
				...
				endforeach;
				foreach($somme_array_risultato as $id_competenza => $dati_competenza):
					$area = get_table('dom_competenze_area', '*', 'deleted=0 and id_dom_competenze_area='.$dati_competenza['titolo']); // tab, order eg array(array("data_validita", "desc")), where
					if($dati_competenza['somma']!=0 && $dati_competenza['conteggio']!=0)
						$media_atteso = round($dati_competenza['somma']/$dati_competenza['conteggio'], 1);
					else $media_atteso =0;
					$vertici_radar_due_risultato[] = array(strtoupper($area[0]["area"]), $media_atteso);
				endforeach;
			...
			<p STYLE="page-break-before: always"></p>
			<h5 style='border-bottom: 1px solid #CCC; margin-bottom:10px;' class='blue'>Radar Competenze Tecnico Specialistiche</h5>
			<div class="row">
            	<div class="col-sm-12">
	...
	for($a=0; $a<count($vertici_radar); $a++):
		if(isset($_REQUEST['idval']))
			$values .= '{ "compA":"'.$vertici_radar[$a][1].'", "compB":"'.$vertici_radar_risultato[$a][1].'", "comp":"'.$lettere[$a].'" },';
		else
	...
	for($a=0; $a<count($vertici_radar_due); $a++):
		if(isset($_REQUEST['idval']))
			$values .= '{ "compA":"'.$vertici_radar_due[$a][1].'", "compB":"'.$vertici_radar_due_risultato[$a][1].'", "comp":"'.$lettere[$a].'" },';
		else
			...
			legend:{layout:"y",width: 400,align:"center",valign:"middle",marker:{width:5,radius:3},
				values:[
					<?php
						...
						echo '{text:"<b>Attesi</b>",color:"#fff"},';
					
						for($a=0; $a<count($vertici_radar); $a++):
							echo '{text:"'.$lettere[$a].') '.$vertici_radar[$a][0]." - ".$vertici_radar[$a][1].'",color:"#3399ff"},';
						endfor;
						echo '{text:"<b>Ottenuti</b>",color:"#fff"},';
						for($a=0; $a<count($vertici_radar_risultato); $a++):
							echo '{ "text":"'.$lettere[$a].') '.$vertici_radar_risultato[$a][0]." - ".$vertici_radar_risultato[$a][1].'",color:"#66cc00"},';
						endfor;
					?>
				]
			}
			...
			legend:{layout:"y",width: 400,align:"center",valign:"middle",marker:{width:5,radius:3},
				values:[
					<?php
						...
						echo '{text:"<b>Ottenuti</b>",color:"#fff"},';
						for($a=0; $a<count($vertici_radar_risultato); $a++):
							echo '{ "text":"'.$lettere[$a].') '.$vertici_radar_due_risultato[$a][0]." - ".$vertici_radar_due_risultato[$a][1].'",color:"#66cc00"},';
							echo '{text:"",color:"#fff"},';
						endfor;
					?>
				]
