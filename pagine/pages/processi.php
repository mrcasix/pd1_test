<?php
	...
	$elenco = get_table('dom_processi', '*', 'deleted=0 '); // tab, order eg array(array("data_validita", "desc")), where

	$data = "[\n";
		for($x=0; $x<count($elenco); $x++):
			$data .= "{";
			$data .= ' "cod_processo": "'.$elenco[$x]['cod_processo'].'",';
			$data .= ' "processo": "'.$elenco[$x]['processo'].'",';
			$data .= ' "button": "<a href=\'index.php?page=sottoprocessi&id='.base64_encode($elenco[$x]['id_dom_processi']).'\' data-rel=\'tooltip\' class=\'tooltip-info\' data-placement=\'bottom\' title=\'Visualizza sottoprocesso\'><i class=\'ace-icon fa fa-sitemap bigger-140 purple\'></i></a>&nbsp;&nbsp;<a href=# id_proc='.$elenco[$x]['id_dom_processi'].' class=\'new_element tooltip-info\' data-placement=\'bottom\' title=\'Modifica processo\'><i class=\'ace-icon fa fa-pencil bigger-140\'></i></a>&nbsp;&nbsp;<a href=# id_proc='.$elenco[$x]['id_dom_processi'].' data-rel=\'tooltip\' class=\'del_element tooltip-danger\' data-placement=\'bottom\' title=\'Cancella processo\'><i class=\'ace-icon fa fa-trash-o bigger-140 red\'></i></a>"';
			$data .= "},\n";
		endfor;
	$data .= "],\n";	
...
<!--	<div class="alert alert-block alert-success">
			...
			<div class="widget-box transparent">
				<div class="widget-header widget-header-flat">
                    ...
                    <div class="widget-toolbar">
                        <div class="btn-group">
                        	<a href="#" role="button" id_proc=0 data-rel='tooltip' class="tooltip-info new_element" data-placement="bottom" title="Nuovo processo">
	...
	jQuery(function($) {
			...
			$(document).on('click', '.new_element', function(){
					...
					var form;					
					$.ajax({
						url: '../build/p_lib/ajax_check.php?op=form_processi',
						type: 'POST',
						data: {cod_processo: cod,
							   processo: title,
							   id: id
							   },
						...
						buttons: {
							"success" : {
								...
								"callback": function() {
									...
									cod_processo =  $("#cod_processo").val();
									$.ajax({
										url: '../build/p_lib/ajax_check.php?op=save_tb&tb=dom_processi',
										type: 'POST',
										data: {id_dom_processi: id,
											   processo: processo,
			...
			$(document).on('click', '.del_element', function(){
				var id = $(this).attr("id_proc");
				var box = bootbox.dialog({
						message: "Sei sicuro di voler eliminare il processo?",
						className: "modal70",
						title:   "Attenzione!",
						buttons: {
							"success" : {
								...
								"callback": function() {
									$.ajax({
										url: '../build/p_lib/ajax_check.php?op=update_tb&tb=dom_processi',
										type: 'POST',
										data: {id_dom_processi: id, deleted: '1'},
										success:function(msg){
											location.reload();
										},
										error: function(response) {},
